...
def cenario(cenario, inimigo):
    print('You are in the mining floor, here you found an aggressive', floor[inimigo].name, 'get ready to fight')
    return inimigo
...
classe = Intinput("choose, Archers, use ranged weapons, Paladins use light close range weapons and barbarians use heavy close range ones", 0, 4)
if classe == 1:
   vitality = 70
   dexterity = 100
   endurance = 60
...
print("Kid - We are Damned Souls that ran away from the Demons and found this village")
...
print("Kid - So you should get to know the town, after that I will show you some tunnels around, wait for me before leaving town ok?")
print("You - Sure")
city.city()
...
if S == 1:
    print("Kid - Here we are, attack that training bot, it wont fight back")
    bot = 25
    while (float(bot) > 0):
            ...
            if S == 2:
                player.stamina -= (equipped.weapon).weight
            player.stamina += player.endurance/30
            print("health:", player.health)
            print("bot health", round(bot))
            print("Your Stamina", player.stamina)
...
print("Kid - These are the Tunnels...")
