...
class player:
    ...
    def DamageGiven(player, weapon, classe):
        ...
        if classe == 2:
            return ((weapon.damage*player.dexterity*player.streght)/(100*100))
        if classe == 3:
            return ((weapon.damage*player.streght)/(100))
...
class armor(itens):
    def __init__(self, name, weight, Type, value, durability, quality, defense, enchantment):
...
def batalha(playerhealth):
    ...
    if a == 0:
        while (float(playerhealth) > 0) and (float(fighter.health) > 0):
            tupla = AI(playerhealth, fighter, tupla[1])
            if tupla[1] == 1:
                print("It protects itself against you!")
            else:
                playerhealth = round(tupla[0])
...
def AI(playerhealth, fighter, defense):
    AI = ran.randint(0,10)
    if AI <= 7:
        fighterdamage = fighter.weapon.damage
        yourresistence = (player.damage_percent_taken())
        if defense == 0:
            playerhealth -= (1-yourresistence)*fighterdamage
            return [playerhealth, 0]
        else:
            playerhealth -= ((1-yourresistence)*fighterdamage)/2
            return [playerhealth, 0]
    else: 
...
#cria roupas iniciais do player self, name, weight, Type, value, durability, quality, defense, enchantment)
head_naked = armor("head_naked", 0, "None", 0, -1, "none", 0, "none")
chothshirt =  armor("chothshirt", 0, "None", 0, -1, "none", 0, "none")
clothttrousers =  armor("clothttrousers", 0, "None", 0, -1, "none", 0, "none")
feet_naked =  armor("feet_naked", 0, "None", 0, -1, "none", 0, "none")
arm_naked =  armor("arm_naked", 0, "None", 0, -1, "none", 0, "none")
hand_naked =  armor("hand_naked", 0, "None", 0, -1, "none", 0, "none")
...
playerhealth = int(player.Start_health())
playerstamina = player.endurance
